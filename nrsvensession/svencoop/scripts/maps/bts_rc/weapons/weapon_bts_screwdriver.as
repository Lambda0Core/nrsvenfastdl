...
enum screwdriver_e
{
	...
	SCREWDRIVER_DRAW,
	SCREWDRIVER_HOLSTER,
	SCREWDRIVER_ATTACK1HIT,
	SCREWDRIVER_ATTACK1MISS,
	SCREWDRIVER_ATTACK2MISS,
	SCREWDRIVER_ATTACK2HIT,
	SCREWDRIVER_ATTACK3MISS,
	SCREWDRIVER_ATTACK3HIT
};
...
const string V_MODEL = "models/bts_rc/weapons/v_screwdriver.mdl";

class weapon_bts_screwdriver : ScriptBasePlayerWeaponEntity, HLWeaponUtils
{
	...
	bool Deploy()
	{
		return self.DefaultDeploy( self.GetV_Model( "models/bts_rc/weapons/v_screwdriver.mdl" ), self.GetP_Model( "models/bts_rc/weapons/p_screwdriver.mdl" ), SCREWDRIVER_DRAW, "squeak", 0, GetBodygroup() );
	}
	...
	bool Swing( int fFirst )
	{
		...
		if ( tr.flFraction >= 1.0 )
		{
			if( fFirst != 0 )
			{
				...
				{
				case 0:
					self.SendWeaponAnim( SCREWDRIVER_ATTACK1MISS, 0, GetBodygroup() ); break;
				case 1:
					self.SendWeaponAnim( SCREWDRIVER_ATTACK2MISS, 0, GetBodygroup() ); break;
				case 2:
					self.SendWeaponAnim( SCREWDRIVER_ATTACK3MISS, 0, GetBodygroup() ); break;
				}
				self.m_flNextPrimaryAttack = g_Engine.time + 0.35;
				// play wiff or swish sound
				g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sd_miss1.wav", 1, ATTN_NORM, 0, 94 + Math.RandomLong( 0,0xF ) );
		...
		else
		{
			...
			{
			case 0:
				self.SendWeaponAnim( SCREWDRIVER_ATTACK1HIT, 0, GetBodygroup() ); break;
			case 1:
				self.SendWeaponAnim( SCREWDRIVER_ATTACK2HIT, 0, GetBodygroup() ); break;
			case 2:
				self.SendWeaponAnim( SCREWDRIVER_ATTACK3HIT, 0, GetBodygroup() ); break;
			}
...
/*					if( pEntity.pev.classname == "func_breakable" )
					{
						pEntity.TraceAttack( m_pPlayer.pev, -7, g_Engine.v_forward, tr, DMG_CLUB );
					}
*/
	// end aone
					...
					{
					case 0:
						g_SoundSystem.EmitSound( m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sd_hitbod1.wav", 1, ATTN_NORM ); break;
					case 1:
						g_SoundSystem.EmitSound( m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sd_hitbod2.wav", 1, ATTN_NORM ); break;
					case 2:
						g_SoundSystem.EmitSound( m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sd_hitbod3.wav", 1, ATTN_NORM ); break;
					}
			...
			if( fHitWorld == true )
			{
				...
				{
				case 0:
					g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sd_hit1.wav", fvolbar, ATTN_NORM, 0, 98 + Math.RandomLong( 0, 3 ) ); 
					break;
				case 1:
					g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sd_hit2.wav", fvolbar, ATTN_NORM, 0, 98 + Math.RandomLong( 0, 3 ) ); 
					break;
				}
			}
...
string GetScrewdriverName()
{
	return "weapon_bts_screwdriver";
}

void RegisterScrewdriver()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "HL_SCREWDRIVER::weapon_bts_screwdriver", GetScrewdriverName() );
	g_ItemRegistry.RegisterWeapon( GetScrewdriverName(), "bts_rc/weapons" );
}
