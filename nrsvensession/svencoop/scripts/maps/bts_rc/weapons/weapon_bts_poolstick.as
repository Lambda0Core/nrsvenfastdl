...
const string V_MODEL = "models/bts_rc/weapons/v_poolstick.mdl";

class weapon_bts_poolstick : ScriptBasePlayerWeaponEntity, HLWeaponUtils
{
	...
	void Precache()
	{
		...
		g_SoundSystem.PrecacheSound( "bts_rc/weapons/cbar_hitbod1.wav" );
		g_SoundSystem.PrecacheSound( "bts_rc/weapons/cbar_hitbod2.wav" );
		g_SoundSystem.PrecacheSound( "bts_rc/weapons/cbar_hitbod3.wav" );
	...
	bool Deploy()
	{
		return self.DefaultDeploy( self.GetV_Model( "models/bts_rc/weapons/v_poolstick.mdl" ), self.GetP_Model( "models/bts_rc/weapons/p_poolstick.mdl" ), POOLSTICK_DRAW, "crowbar", 0, GetBodygroup() );
	}
	...
	bool Swing( int fFirst )
	{
		...
		if ( tr.flFraction >= 1.0 )
		{
			if( fFirst != 0 )
			{
				...
				{
				case 0:
					self.SendWeaponAnim( POOLSTICK_ATTACK1MISS, 0, GetBodygroup() ); break;
				case 1:
					self.SendWeaponAnim( POOLSTICK_ATTACK2MISS, 0, GetBodygroup() ); break;
				case 2:
					self.SendWeaponAnim( POOLSTICK_ATTACK1MISS, 0, GetBodygroup() ); break;
				}
		...
		else
		{
			...
			{
			case 0:
				self.SendWeaponAnim( POOLSTICK_ATTACK1HIT, 0, GetBodygroup() ); break;
			case 1:
				self.SendWeaponAnim( POOLSTICK_ATTACK2HIT, 0, GetBodygroup() ); break;
			case 2:
				self.SendWeaponAnim( POOLSTICK_ATTACK1HIT, 0, GetBodygroup() ); break;
			}
			...
			if( pEntity !is null )
			{
				...
				{
					self.m_flNextPrimaryAttack = g_Engine.time + 0.4; //0.25
				}
	...
	// end aone
			...
			if( fHitWorld == true )
			{
				...
				{
				case 0:
					g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "debris/wood1.wav", fvolbar, ATTN_NORM, 0, 98 + Math.RandomLong( 0, 3 ) ); 
					break;
				case 1:
					g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "debris/wood2.wav", fvolbar, ATTN_NORM, 0, 98 + Math.RandomLong( 0, 3 ) ); 
					break;
				}
			}
...
string GetPoolstickName()
{
	return "weapon_bts_poolstick";
}

void RegisterPoolstick()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "HL_POOLSTICK::weapon_bts_poolstick", GetPoolstickName() );
	g_ItemRegistry.RegisterWeapon( GetPoolstickName(), "bts_rc/weapons" );
}
