...
HookReturnCode bts_PlayerSpawn(CBasePlayer@ pPlayer)
{
...
HookReturnCode bts_PlayerKilled( CBasePlayer@ pPlayer, CBaseEntity@ pAttacker, int iGib )
{
	...
	else if ( Constr.find(modelName) >= 0 ) // construction death sound
	{
		if( pPlayer.pev.health > -60 )
		{
			if( pPlayer.pev.waterlevel == WATERLEVEL_HEAD )
...
HookReturnCode bts_PlayerPostThink( CBasePlayer@ pPlayer )
{
	bts_PlayPlayerPainSounds( EHandle(pPlayer) );

	return HOOK_CONTINUE;
}

void bts_PlayPlayerPainSounds(EHandle& in ePlayer)
{
